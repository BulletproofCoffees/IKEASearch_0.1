...
public class tt {
	...
	public tt(boolean isGet, Hashtable<String, String> params, int key_count)
	...
	public tt(boolean isGet, Hashtable<String, String> params, String accessSharedSecret, int key_count)
			throws UnsupportedEncodingException, SignatureException {
	...
	// oauth_signature_method String The method used to generate the signature (only
	// HMAC-SHA1 is supported)
	private static String oauthSignatureMethod = "HMAC-SHA1";
	// oauth_timestamp Int The date and time, expressed in the number of seconds
	// since January 1, 1970 00:00:00 GMT. The timestamp value must be a positive
	// integer and must be equal or greater than the timestamp used in previous
	// requests
	private long oauthTimestamp;
	// oauth_nonce String A randomly generated string for a request that can be
	// combined with the timestamp to produce a unique value
	private String oauthNounce;
	// oauth_version String MUST be "1.0"
	private static String oauthVersion = "1.0";
	// oauth_signature String The signature, a consistent reproducible concatenation
	// of the request elements into a single string. The string is used as an input
	// in hashing or signing algorithms.
	...
	*//**
	...
	private String BuildSignature(boolean isGet, Hashtable<String, String> params, int key_count)
			throws UnsupportedEncodingException, SignatureException {
		...
		oauthSignature = calculateRFC2104HMAC(signatureBaseString,
				consumerSharedSecret[key_count] + "&" + accessSharedSecret);
	...
	private void PrepareParameters(Hashtable<String, String> params, int key_count) {
		...
		// Please refer to the FatSecret REST API documentation for the full parameter
		// list for each method, but for OAuth authentication the following parameters
		...
		// oauth_nonce
		// A randomly generated string for a request that can be combined with the
		// timestamp to produce a unique value
		// oauth_timestamp
		// The date and time, expressed in the number of seconds since January 1, 1970
		// 00:00:00 GMT. The timestamp value must be a positive integer and must be
		...
		// lexicographical byte value ordering, first by name and then by value. Finally
		// the parameters are concatenated in their sorted order into a single string,
		// each name-value pair separated by an '&' character (ASCII code 38).
